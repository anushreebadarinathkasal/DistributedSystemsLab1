...
public class Client {
	...
	// Client constructor class takes IP which is IPaddress (127.0.0.1)and portno(8818)and the userName of client. 
	...
	public void startClient() throws UnknownHostException, IOException, InterruptedException {
		...
		while (true) {
			...
			writeToServer(wr, "\n*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*");
			//message to be printed in between *-*-* in order to differentiate one from another
			writeToServer(wr, req);
	...
	private String readResponseDataFromServer(BufferedReader reader) throws IOException {
				...
				while (true) {
					...
					if (line == null)
						...
						// If the line is \r\n or the line is empty the header data is completed the next data is the data to be read.
						if (line.equals("\r\n") || line.equals("")) {
